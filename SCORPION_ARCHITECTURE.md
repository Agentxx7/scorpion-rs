...
| Web Console | `scorpion_app` | `spider::features::domain_runtime`, `spider::features::search`, `spider::agent`, `spider::utils::evidence::{EvidenceRef::resolve, read_evidence}` (`scorpion_app/src/evidence.rs` only — see §3.19), audit module/`audit_page` (`scorpion_app/src/audit.rs` only — see §3.19), `spider::utils::evidence::{fetch_single_page_with_options, build_evidence, record_evidence}` (`scorpion_app/src/fetch.rs` only, `TransportPolicy::Default` fixed — no Tor selection exposed) | `reqwest::Client` construction, independent domain logic, independent evidence reconstruction, independent audit rule/technology-marker assembly, `fetch_single_page` inside `audit.rs` specifically, `Website::`/crawl-family construction inside `fetch.rs` specifically, any Tor/transport-selection surface inside `fetch.rs` | `scorpion-api` binary (`GET /`, `GET /health`, `POST /api/search`, `POST /api/research`, `GET /api/research/{id}`, `GET /api/evidence/{evidence_ref}`, `GET /api/evidence/{evidence_ref}/content`, `GET /api/evidence/{evidence_ref}/export`, `POST /api/audit`, `POST /api/fetch`) | None |
