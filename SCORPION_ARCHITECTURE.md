...
| Web Console | `scorpion_app` | `spider::features::domain_runtime`, `spider::features::search`, `spider::agent`, `spider::utils::evidence::{EvidenceRef::resolve, read_evidence}` (`scorpion_app/src/evidence.rs` only — see §3.19), audit module/`audit_page` (`scorpion_app/src/audit.rs` only — see §3.19), `spider::utils::evidence::{fetch_single_page_with_options, build_evidence, record_evidence}` (`scorpion_app/src/fetch.rs` only, `TransportPolicy::Default` fixed — no Tor selection exposed), `spider::features::iam_trace`/`identity::IamTraceId`/`domain_state`/`domain_persistence` (`scorpion_app/src/iam.rs` only — see §3.8; bounded decode-only OAuth2/OIDC/JWT inspection via the `base64` crate, `scorpion_app/Cargo.toml` only; bounded decode-only SAML 2.0 structural inspection via `spider::quick_xml` — already re-exported, no new dependency — no JWKS/X.509/`jsonwebtoken`/OAuth-client/SAML/XML-signature crate anywhere in the workspace) | `reqwest::Client` construction, independent domain logic, independent evidence reconstruction, independent audit rule/technology-marker assembly, `fetch_single_page` inside `audit.rs` specifically, `Website::`/crawl-family construction inside `fetch.rs` specifically, any Tor/transport-selection surface inside `fetch.rs`, any cryptographic signature verification (JWT or SAML), X.509 handling, or SAML login/AuthnRequest implementation inside `iam.rs` specifically | `scorpion-api` binary (`GET /`, `GET /health`, `POST /api/search`, `POST /api/research`, `GET /api/research/{id}`, `GET /api/evidence/{evidence_ref}`, `GET /api/evidence/{evidence_ref}/content`, `GET /api/evidence/{evidence_ref}/export`, `POST /api/audit`, `POST /api/fetch`, `POST /api/iam/traces`, `GET /api/iam/traces/{trace_id}`, `GET`/`POST /iam/callback/{trace_id}`) | None |
...
| IAM Callback Inspector | **PARTIALLY BLOCKED** | Canonical operator-facing diagnostic capability for inspecting what an IAM/federation/IdP/application flow actually sent and received during a callback/redirect exchange (real IAM troubleshooting, not a protocol implementation). See clarification below for the full decision record — V1 protocol scope, the required OBSERVED/VALIDATED/NOT_VALIDATED/REDACTED fact model, and the security contract. `SCORPION_CANONICAL_IAM_TRACE_AND_OBSERVATION_MODEL_001` implemented the pure canonical model — `IamTraceId` (`features/identity.rs`), the `AwaitingCallback`/`Received` trace lifecycle (`features/iam_trace.rs`, built on `domain_state::Transition`), the provider-neutral `IamCallbackObservation`/`IamFact` shape, the `IamFactStatus` OBSERVED/VALIDATED/NOT_VALIDATED/REDACTED vocabulary, and the SHA-256 redaction helper. `SCORPION_IAM_CALLBACK_RECEIVER_AND_PERSISTENCE_001` then implemented the first real runtime slice on the existing `scorpion-api` listener — trace creation (`POST /api/iam/traces`), the passive generic callback receiver (`GET`/`POST /iam/callback/{trace_id}`, query/form/JSON only, sensitive fields redacted to a SHA-256 digest, never `Validated`), durable persistence through the existing `DomainPersistence`/`domain_runtime` mechanism (no second database), and readback (`GET /api/iam/traces/{trace_id}`) — see §3.7's Web Console row for the exact routes. `SCORPION_IAM_CALLBACK_TRACE_STATE_RECOVERY_001` closed the one remaining crash-consistency window: an observation durably appended but the `AwaitingCallback -> Received` transition never persisted, with no later callback ever arriving to trigger the prior self-healing retry. A real single-transaction fix was evaluated and rejected — it would force a legitimate concurrent state conflict to roll back the observation itself, and pushing transition logic into `DomainPersistence` would cross `domain_state.rs`'s own persistence/domain boundary. Instead, `read_trace` itself now repairs the condition (durable history non-empty + state still `AwaitingCallback` → attempt and persist the transition before returning), via the exact same `write_current` path, before every readback — never a value fabricated only in one response. `SCORPION_IAM_OIDC_AND_JWT_INSPECTION_001` then added bounded, decode-only OAuth2/OIDC/JWT inspection: callbacks are classified `Generic`/`OAuth2`/`Oidc` from observed parameter names only (`id_token` presence always wins; otherwise `code`/`error`/`state` → `OAuth2`), an observed `id_token` is decoded transiently (never persisted raw) into `jwt.header.{alg,typ,kid}` and `jwt.{iss,aud,exp,iat,nbf}` Observed facts plus `jwt.nonce`/`jwt.signature.verification` `NotValidated` facts (no expectation-storage model exists for `state`/`nonce` either — both are relabeled `NotValidated` using the existing vocabulary, not a new field), `jwt.signature.present` Observed truthfully, and a malformed/oversized token yields one truthful `jwt.decode_error` diagnostic fact while the callback observation itself, `id_token`'s own `Redacted` fact, and the `Oidc` classification all remain intact regardless. `access_token`/`code`/`refresh_token`/`client_secret` remain `Redacted`, unconditionally, and are never JWT-decoded even when JWT-shaped. `SCORPION_IAM_SAML_RESPONSE_INSPECTION_001` then added bounded, decode-only SAML 2.0 Response/Assertion structural inspection for the HTTP-POST binding (`application/x-www-form-urlencoded`, `SAMLResponse=<base64 XML>`) via `spider::quick_xml` (already a re-exported workspace dependency — no new one): `SAMLResponse` presence classifies `Saml` (highest precedence — wins over `id_token`/`code`/`error`/`state` in the unlikely event more than one artifact appears in one callback), the raw base64 is decoded transiently and never persisted (`quick_xml::Reader` performs no DTD/entity resolution and no external I/O of any kind — XXE is structurally impossible here, not merely disabled), and `saml.response.{id,destination,version}`, `saml.issuer`, `saml.status.{code,message}`, `saml.assertion.{id,issue_instant}`, `saml.name_id`(`.format`), `saml.conditions.{not_before,not_on_or_after}`, `saml.audience`, `saml.attribute.name`/`saml.attribute.value[…]`, and `saml.{response,assertion}.signature.present` are surfaced Observed (bounded, and still redacted if a name collides with the existing sensitive-name list); `saml.response.in_response_to` is `NotValidated` (a pure anti-replay correlation token, no expectation model exists to compare it against — same rationale as OIDC's `state`/`nonce`, nothing new invented); `saml.signature.verification` is unconditionally `NotValidated`; a malformed/oversized `SAMLResponse` yields one truthful `saml.decode_error` diagnostic fact while the observation, the `Redacted` `SAMLResponse` fact, and the `Saml` classification all remain intact. NameID and attribute values are surfaced in plaintext — an explicit, documented decision (not silently invented): the original owner decision named both as required V1 diagnostic facts, and that frontier's own owner-runtime procedure uses a real-shaped NameID and a literal test attribute expecting them observable. `SCORPION_IAM_CALLBACK_INSPECTOR_WEB_UI_001` then added the first operator-facing rendering layer: a new "IAM Callback Inspector" section inside the existing Web Console (`scorpion_app/src/main.rs`'s `INDEX_HTML`/inline script only — no new file, no new dependency, no new backend route) exposing exactly the three already-shipped routes above (`POST /api/iam/traces`, `GET /api/iam/traces/{trace_id}`, the passive `GET`/`POST /iam/callback/{trace_id}` receiver, unchanged) — New Trace, a copyable callback URI, trace-state display, client-side polling (bounded, `setTimeout`-based, no background server scheduling) until the first observation arrives, and a Refresh control for any later ones. Facts render through a fixed, presentation-only partition — Callback/OAuth-OIDC, JWT, SAML, Validation (any `NotValidated`/`Validated` fact by name-independent status, e.g. the raw OAuth2 `state` parameter alongside `jwt.nonce`/`saml.response.in_response_to`), Redacted material (any `Redacted` fact by status, regardless of name) — that adds no new IAM classification of its own, only groups facts scorpion_app::iam already computed. OBSERVED/VALIDATED read as neutral/confirmed; NOT_VALIDATED/REDACTED read as an explicit warning/inert tone that never shares a color with a success state; a `Redacted` fact's digest is the only field ever read off that status variant (there is no plaintext field to read); every value renders through the same `createTextNode`-based `text()` helper the rest of the console already uses — no `innerHTML`/`atob`/`DOMParser`/manual JWT-or-XML decoding exists anywhere in the console's JavaScript, so no raw `id_token`/`SAMLResponse` material is ever reconstructed client-side. It must still state: JWT/SAML cryptographic signature verification absent (every signature fact is `NotValidated`, never `Validated`, by construction — no key lookup, no JWKS, no X.509, no XML canonicalization, no algorithm-driven behavior of any kind), a canonical `state`/`nonce`/`InResponseTo` *expectation* model absent (audited and explicitly not invented across both the OIDC and SAML frontiers), any public relay/tunnel absent, and real IdP interoperability not yet owner/live verified (only synthetic local JWT/SAML material has been exercised, now including through the browser UI itself). |
| Installable Application (Windows-first distribution) | **PACKAGED — PENDING OWNER WINDOWS INSTALL** | Scorpion is distributable as a normal installable end-user application on Windows: a real MSI, built via the WiX Toolset (through `cargo-wix`) from `scorpion_app/wix/main.wxs`, installs `scorpion-api.exe` and a new thin startup orchestrator, `scorpion-launcher.exe` (`scorpion_app/src/bin/scorpion_launcher.rs`), side by side, plus a Start Menu entry and a Desktop shortcut, both launching `scorpion-launcher.exe` — never `scorpion-api.exe` directly, never a terminal command. `SCORPION_WINDOWS_INSTALLABLE_APPLICATION_001` implemented this as a distribution/runtime boundary only, reusing the existing `scorpion_app`/`scorpion-api` architecture exactly as-is (§3.7's Web Console row, IAM Callback Inspector included) — no second backend, no rewritten Web Console, no embedded WebView (the OS default browser is used). The launcher: resolves `%LOCALAPPDATA%\Scorpion` as the writable application-data directory (creating it and its `logs` subdirectory if absent — never writes under its own read-only install directory); configures the existing canonical `DomainPersistence`/`domain_runtime` mechanism for the spawned `scorpion-api` child by setting the one canonical `SCORPION_DOMAIN_DB` env var internally (reusing `spider::features::domain_runtime::DOMAIN_DATABASE_ENV`, never a locally re-typed literal or a second persistence system) to `%LOCALAPPDATA%\Scorpion\scorpion.sqlite3`; redirects the child's stdout/stderr to `%LOCALAPPDATA%\Scorpion\logs\scorpion-api.log` (proven, by a real end-to-end test, to never contain a raw redacted secret — the existing IAM redaction rules were already sufficient, nothing about them changed); binds only `127.0.0.1:8787` (never `0.0.0.0`, never a different port — the existing IAM callback-URI architecture has no end-to-end dynamic-port propagation); probes `GET /health` truthfully before ever opening a browser, distinguishing "already-healthy Scorpion, reuse it" from "port free, spawn one" from "port occupied by something else, fail visibly, never guess a different port, never kill the unrelated process"; and shows a truthful operator-facing error (a native dialog via `powershell.exe`'s `System.Windows.Forms.MessageBox`, always backed by the guaranteed `launcher.log` diagnostic record) rather than ever opening a browser on a fabricated success. Zero new Cargo dependencies were needed for the launcher itself (`std` alone: `windows_subsystem`, `CommandExt::creation_flags`/`CREATE_NO_WINDOW`, `cmd /C start` for the browser, `%LOCALAPPDATA%` via `std::env::var`). Uninstall is the normal Windows MSI uninstall (Programs & Features) — it removes the installed binaries/shortcuts only; `%LOCALAPPDATA%\Scorpion` (the database and logs) is outside the MSI-managed install directory entirely, so user data is preserved by default with no extra logic required. A new CI workflow, `.github/workflows/windows-package.yml`, builds and tests the exact release binaries on a genuine `windows-latest` GitHub Actions runner and packages them into a real, checksummed `.msi` artifact (using WiX Toolset v3.14.1, documented as already present on that runner image, with a pinned-and-checksum-verified direct-download fallback if it is ever absent) — a green run there means the artifact was really built and its own bundled test suite passed on Windows, not that a human has installed or launched it. Process-lifecycle limitation, reported not hidden: the launcher does not remain resident supervising `scorpion-api` for the whole "application session" (that would require either a system-tray application or a Windows Service, both explicitly out of scope) — it spawns/reuses, waits for health, opens the browser, and exits; a second launch still deterministically avoids a duplicate server via the same health-probe-first logic, but there is no "Quit Scorpion" control in this V1, and `scorpion-api` keeps running independently until the user's session ends or it is stopped some other way. It must still state truthfully as absent: production code signing (an unsigned installer — no code-signing certificate exists in this environment), auto-update, a Windows Service, an embedded native WebView, a public relay/tunnel, real IdP interoperability, and cryptographic JWT/XML-DSig verification (all unchanged from the IAM Callback Inspector's own already-documented gaps above). This frontier does not become `OWNER_VERIFIED_USABLE` until jonny personally installs and launches the real `.msi` on a real Windows machine — see this frontier's closure report for the exact procedure. A real MSI has actually been built and verified by `windows-latest` GitHub Actions CI (run `33972458053`, commit `ed9506fd3e05e64a7daba6268269d20c1d627f4f`): artifact `scorpion_app-2.53.4-x86_64.msi`, SHA-256 `8578692a99c7317448d14b64820786b826f28e23305b1c509e4e30ec6f2f59c6`, uploaded as the `scorpion-windows-installer` workflow artifact — this is TESTED/ARTIFACT_BUILT, not LIVE_VERIFIED or OWNER_VERIFIED. Getting to a real Windows build/link surfaced and fixed three genuine, pre-existing, unrelated cross-platform gaps nobody had ever hit before (nothing in this workspace had ever been compiled for Windows prior to this frontier): `spider_transport`'s `ResolvedExecutor::resolve` called `reqwest::ClientBuilder::interface()` with no platform guard at all (reqwest itself only defines that method on a fixed non-Windows platform set) — fixed by mirroring the exact cfg-gate pattern and no-op-on-unsupported-platform fallback already established elsewhere in the same file and in `spider::website::set_interface`; two of this frontier's own new tests only set `HOME`, never `LOCALAPPDATA`, so they passed on Linux but read the real system app-data directory instead of their own isolated fixture on real Windows; and `scorpion_app/tests/cross_interface_evidence.rs`'s on-demand `spider-mcp`/`scorpion` binary locators were missing the `.exe` suffix Windows requires. Separately, and *not* fixed under this frontier: a pre-existing, intermittent test-fixture-reliability gap was found and reproduced three times across different real Windows CI runs — a *different* one of scorpion_app's own tests each time (never anything this frontier added) failed with the identical "a locally listening raw-socket HTTP test fixture was reported unreachable" symptom, evidently a race between that shared fixture pattern's background accept-loop thread and Windows CI scheduling. `.github/workflows/windows-package.yml` deliberately does not let that gate MSI packaging (every packaging step is conditioned on the binaries actually having built, not on that test step's own outcome) but still surfaces it honestly as a job failure rather than hiding it — hardening that shared fixture pattern with real readiness synchronization is a real, separate, scoped follow-up, not invented or patched piecemeal here. |
...
OBSERVATION_MODEL_001` (see the table row above) implemented the pure
canonical trace/observation/redaction model this record describes, and
`SCORPION_IAM_CALLBACK_RECEIVER_AND_PERSISTENCE_001` then implemented the
generic callback receiver, redaction wiring, and durable persistence/
readback on the existing `scorpion-api` listener (again, see the table row
above for the exact routes and what remains explicitly not implemented:
OIDC/JWT and SAML decoding, cryptographic signature verification, the Web
UI Inspector section, and any public callback relay/tunnel).
...
Evidence) built on the capability decided above — this UX target is now
implemented (see the IAM Callback Inspector row above,
`SCORPION_IAM_CALLBACK_INSPECTOR_WEB_UI_001`) and reachable through the
installed application exactly as decided. Explicitly out of scope for
this decision: implementation, installer technology selection, and
auto-update implementation — installer technology selection and the
first real implementation were made by
`SCORPION_WINDOWS_INSTALLABLE_APPLICATION_001` (see this table's own
row above for what exists); auto-update remains undecided and
unimplemented.
