...
async fn run_trial(provider: &PaligemmaLocalCaptchaProvider, fixture: &Fixture) -> Trial {
    let request = CaptchaSolveRequest {
        correlation_id: format!("paligemma-precision-matrix-{}", fixture.label),
...
/// Real paligemma-local point-selection precision, across the full required
/// position matrix, through the exact production `CaptchaProvider::solve`
/// seam.
#[tokio::test]
#[ignore = "requires the pinned ~11.7 GB PaliGemma installation and a qualified CPU/F32 host"]
async fn real_point_selection_precision_matrix() {
    ...
    assert!(
        ...
        "must clear the predefined reliable-single-shot threshold, fixed before this trial ran"
    );
