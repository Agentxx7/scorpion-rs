...
use spider::features::qwen3_vl_runtime::{qwen3_vl_cpu_f32_manifest, qwen3_vl_cpu_f32_manifest_4b};
...
/// The 2B checkpoint's own measured containment rate
/// (`SCORPION_QWEN3_VL_LOCAL_CAPTCHA_POINT_PRECISION_AND_INPUT_ENVELOPE_001`),
/// out of the 7 standard-size fixtures — the baseline this frontier's
/// candidate model must beat.
const BASELINE_2B_CONTAINED: usize = 3;
const STANDARD_FIXTURE_COUNT: usize = 7;
/// Predefined reliability bar for declaring a candidate model qualified for
/// single-shot actionable `PointSelection`, fixed before any 4B trial ran
/// (`SCORPION_QWEN3_VL_LOCAL_CAPTCHA_HIGHER_CAPACITY_MODEL_QUALIFICATION_001`).
/// This is a fail-closed, no-retry, security-relevant single browser action
/// (see the frontier's own NO list: no retry, no voting, no ensemble) —
/// 6 of 7 (~86%) is the practical minimum for that to be trustworthy
/// un-aided, deliberately well above a "marginal improvement" like 4/7,
/// which the frontier explicitly says must not be declared qualified.
...
/// Run the complete required position matrix against `provider`, prove
/// determinism, structured-output validity and genuine anti-degenerate
/// position dependence, print the full report, and return the standard-size
/// (44px) actionable-region containment count out of
/// [`STANDARD_FIXTURE_COUNT`]. Shared verbatim by every checkpoint's own
/// qualification test so the comparison across checkpoints is directly
/// meaningful — identical fixtures, identical tolerance, identical
/// methodology, identical assertions.
async fn run_qualification_matrix(provider: &Qwen3VlLocalCaptchaProvider) -> usize {
...
async fn real_point_selection_precision_matrix() {
    ...
    let contained_count = run_qualification_matrix(&provider).await;
    assert_eq!(contained_count, BASELINE_2B_CONTAINED);
    eprintln!(
...
/// Real qwen3-vl-local point-selection precision for the higher-capacity 4B
/// checkpoint (`SCORPION_QWEN3_VL_LOCAL_CAPTCHA_HIGHER_CAPACITY_MODEL_QUALIFICATION_001`),
/// through the identical methodology, fixtures, tolerance and assertions as
/// [`real_point_selection_precision_matrix`] — the comparison is directly
/// meaningful because nothing about the measurement itself differs, only
/// the checkpoint under test. Reports, but does not itself gate on, whether
/// the candidate clears the predefined [`RELIABLE_CONTAINMENT_THRESHOLD`];
/// the frontier's own closure decision is made from this evidence, not
/// encoded as a hard pass/fail here, exactly as the 2B baseline test above
/// does not hard-gate on its own (already-classified-insufficient) accuracy.
#[tokio::test]
#[ignore = "requires the pinned ~8.3 GB 4B Qwen3-VL installation and a qualified CPU/F32 host"]
async fn real_4b_point_selection_precision_matrix() {
    let source = PathBuf::from(
    ...
    eprintln!(
        "\nstandard-size (44px) actionable containment: {contained_count}/{STANDARD_FIXTURE_COUNT} \
         (2B baseline: {BASELINE_2B_CONTAINED}/{STANDARD_FIXTURE_COUNT}; \
         reliable-single-shot threshold: {RELIABLE_CONTAINMENT_THRESHOLD}/{STANDARD_FIXTURE_COUNT})"
    );
    eprintln!(
        "material improvement over 2B baseline: {}",
        contained_count > BASELINE_2B_CONTAINED
    );
    eprintln!(
        "meets predefined reliable-single-shot threshold: {}",
