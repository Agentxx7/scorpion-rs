...
/// Qwen3-VL's rotary embedding is NOT a plain sequential 1D scheme: the
/// `rope_scaling.mrope_section`/`mrope_interleaved` config keys select an
/// interleaved 3-axis (temporal, height, width) frequency layout so image
/// tokens carry genuine 2D spatial position information rather than a flat
/// index. `position_ids` (one `[t, h, w]` triple per sequence position,
/// identical across axes for text tokens) must be supplied by the caller —
/// this module has no visibility into which spans are image tokens.
#[derive(Debug, Clone)]
...
impl RotaryEmbedding {
    ...
    /// Interleaved MRoPE frequency selection, mirroring the reference
    /// `apply_interleaved_mrope`: start from the temporal-axis frequency at
    /// every index, then overwrite indices `1, 4, 7, ...` (up to
    /// `mrope_section[1] * 3`) with the height-axis frequency and indices
    /// `2, 5, 8, ...` (up to `mrope_section[2] * 3`) with the width-axis
    /// frequency. The remaining (untouched) indices stay temporal — exactly
    /// `mrope_section[0]` of them by construction.
    fn cos_sin(
        &self,
        position_ids: &[[i64; 3]],
        dtype: DType,
        ...
        let h_len = self.mrope_section[1] * 3;
        let w_len = self.mrope_section[2] * 3;
        for pos in position_ids {
            let mut row = vec![0f32; half];
            for (i, value) in row.iter_mut().enumerate() {
                *value = pos[0] as f32 * self.inv_freq[i];
            }
            let mut i = 1;
            while i < h_len && i < half {
                row[i] = pos[1] as f32 * self.inv_freq[i];
                i += 3;
            }
            let mut i = 2;
            while i < w_len && i < half {
                row[i] = pos[2] as f32 * self.inv_freq[i];
                i += 3;
            }
            // `candle_nn::rotary_emb::rope` expects cos/sin sized to half the
            // head dim and doubles internally (unlike the reference's own
            // manual `cat((freqs, freqs))`, which feeds a different, from-
            // scratch rope application) — do not double here.
            for value in &row {
                cos.push(value.cos());
                sin.push(value.sin());
            }
        }
        let cos = Tensor::from_vec(cos, (seq_len, half), device)?.to_dtype(dtype)?;
        let sin = Tensor::from_vec(sin, (seq_len, half), device)?.to_dtype(dtype)?;
        Ok((cos, sin))
    }
    ...
    pub fn forward(
        ...
        let (b_sz, _qh, _seq_len, _n_embd) = q.dims4()?;
        debug_assert_eq!(b_sz, 1, "this runtime only ever calls with batch size 1");
        let (cos, sin) = self.cos_sin(position_ids, q.dtype(), q.device())?;

        let rope = candle_nn::rotary_emb::rope;
        let q_embed = rope(&q.i(0)?.unsqueeze(0)?.contiguous()?, &cos, &sin)?;
        let k_embed = rope(&k.i(0)?.unsqueeze(0)?.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
impl Attention {
    ...
    fn forward(
        ...
        (q, k) = self.rotary_emb.forward(&q, &k, position_ids)?;
...
impl Qwen3VLTextModel {
    pub fn new(cfg: &TextConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let mrope_section = cfg
            .rope_scaling
            .as_ref()
            .and_then(|scaling| scaling.mrope_section)
            .unwrap_or([24, 20, 20]);
