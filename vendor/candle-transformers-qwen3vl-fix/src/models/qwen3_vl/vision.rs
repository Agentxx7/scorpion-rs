...
impl Qwen3VLVisionModel {
    ...
    pub fn forward(&self, xs: &Tensor, grid_thw: &Tensor) -> Result<(Tensor, Vec<Tensor>)> {
        ...
        crate::models::qwen3_vl::audit_dump("vision.patch_embed", &xs);
        let pos_embeds = self.fast_pos_embed_interpolate(grid_thw)?;
        crate::models::qwen3_vl::audit_dump("vision.pos_embeds", &pos_embeds);
        let mut hidden_states = xs.add(&pos_embeds)?;
        crate::models::qwen3_vl::audit_dump("vision.after_pos_add", &hidden_states);
        ...
        for (layer_idx, block) in self.blocks.iter().enumerate() {
            ...
            if layer_idx == 0 {
                crate::models::qwen3_vl::audit_dump("vision.block0_out", &hidden_states);
            }
            if let Some(merger_idx) = self.deepstack_lookup[layer_idx] {
                let feat = self.deepstack_mergers[merger_idx].forward(&hidden_states)?;
                crate::models::qwen3_vl::audit_dump(
                    &format!("vision.deepstack[{merger_idx}]"),
                    &feat,
                );
                deepstack_features.push(feat);
            }
        }
        crate::models::qwen3_vl::audit_dump("vision.final_block_out", &hidden_states);

        let hidden_states = self.merger.forward(&hidden_states)?;
        crate::models::qwen3_vl::audit_dump("vision.merger_out", &hidden_states);
