...
use crate::handler::{
    AttachedSessionState, Handler, HandlerConfig, HandlerMessage, REQUEST_TIMEOUT,
};
...
use chromiumoxide_cdp::cdp::browser_protocol::target::{
    ...
    GetBrowserContextsParams, GetBrowserContextsReturns, SessionId, TargetId, TargetInfo,
};
...
/// Failure while resolving, validating, or commanding a flattened attached
/// target session.
#[derive(Debug)]
pub enum AttachedSessionError {
    /// Chromium knows the target, but it has no active attached session.
    TargetNotAttached,
    /// The captured session detached.
    SessionDetached,
    /// The target now uses a different session generation.
    SessionReplaced,
    /// Chromium destroyed the target.
    TargetDestroyed,
    /// The target is unknown to this browser connection generation.
    UnknownTarget,
    /// The browser connection or command route failed.
    CommandRoutingFailed(CdpError),
}

impl std::fmt::Display for AttachedSessionError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::CommandRoutingFailed(error) => {
                write!(formatter, "command routing failed: {error}")
            }
...
impl std::error::Error for AttachedSessionError {}

/// Immutable command-capable handle for one flattened target attachment.
///
/// The handle never follows a target to a replacement `SessionId`.
#[derive(Debug)]
pub struct AttachedTargetSession {
    target_id: TargetId,
    session_id: SessionId,
    sender: Sender<HandlerMessage>,
}

impl AttachedTargetSession {
    /// Owning Chromium target identity.
    pub fn target_id(&self) -> &TargetId {
        &self.target_id
    }

    /// Exact immutable flattened attachment identity.
    pub fn session_id(&self) -> &SessionId {
        &self.session_id
    }

    /// Verify that this exact target/session association is still current.
    pub async fn validate(&self) -> std::result::Result<(), AttachedSessionError> {
        let (tx, rx) = oneshot_channel();
        self.sender
            .send(HandlerMessage::GetAttachedSession(
        ...
        let state = rx.await.map_err(|error| {
            AttachedSessionError::CommandRoutingFailed(CdpError::msg(error.to_string()))
        })?;
        match state {
            ...
            AttachedSessionState::UnknownTarget => Err(AttachedSessionError::UnknownTarget),
        }
    }

    /// Execute a CDP command through this exact attached session.
    pub async fn execute<T: Command>(
        &self,
        command: T,
    ) -> std::result::Result<CommandResponse<T::Response>, AttachedSessionError> {
        self.validate().await?;
        let (tx, rx) = oneshot_channel();
        let method = command.identifier();
        let message = CommandMessage::with_session(command, tx, Some(self.session_id.clone()))
            .map_err(|error| AttachedSessionError::CommandRoutingFailed(CdpError::Serde(error)))?;
        self.sender
            .send(HandlerMessage::Command(message))
            .await
            .map_err(|error| {
                AttachedSessionError::CommandRoutingFailed(CdpError::msg(error.to_string()))
            })?;
        let response = rx
            ...
            .map_err(AttachedSessionError::CommandRoutingFailed)?;
        to_command_response::<T>(response, method)
            .map_err(AttachedSessionError::CommandRoutingFailed)
    }

    /// Subscribe to events scoped by chromey to this exact session.
    pub async fn event_listener<T: IntoEventKind>(
        &self,
    ) -> std::result::Result<EventStream<T>, AttachedSessionError> {
        self.validate().await?;
        let (tx, rx) = unbounded_channel();
        self.sender
            .send(HandlerMessage::AddSessionEventListener(
...
impl Browser {
    ...
    /// Return a command-capable immutable handle for an actively attached
    /// flattened target session (including an OOPIF target).
    pub async fn attached_session(
        &self,
        target_id: TargetId,
    ) -> std::result::Result<AttachedTargetSession, AttachedSessionError> {
        let (tx, rx) = oneshot_channel();
        self.sender
            .send(HandlerMessage::GetAttachedSession(target_id.clone(), tx))
            .await
            .map_err(|error| {
                AttachedSessionError::CommandRoutingFailed(CdpError::msg(error.to_string()))
            })?;
        ...
        match state {
            ...
            AttachedSessionState::NotAttached => Err(AttachedSessionError::TargetNotAttached),
            AttachedSessionState::TargetDestroyed => Err(AttachedSessionError::TargetDestroyed),
            AttachedSessionState::UnknownTarget => Err(AttachedSessionError::UnknownTarget),
        }
    }
