...
pub struct Handler {
    ...
    /// Target identities destroyed during this browser connection generation.
    destroyed_targets: HashSet<TargetId>,
    ...
    /// Event listeners filtered by exact flattened attachment session.
    session_event_listeners: HashMap<SessionId, EventListeners>,
...
impl Handler {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            destroyed_targets: Default::default(),
            ...
            session_event_listeners: Default::default(),
            closing: false,
    ...
    fn on_event(&mut self, event: CdpEventMessage) {
        if let Some(session_id) = &event.session_id {
            if let Some(listeners) = self.session_event_listeners.get_mut(session_id.as_str()) {
                let params = event.params.clone();
                let method = event.method.clone();
                chromiumoxide_cdp::consume_event!(match params {
                    |ev| listeners.start_send(ev),
                    |json| { let _ = listeners.try_send_custom(&method, json); }
                });
            }
    ...
    fn on_target_created(&mut self, event: EventTargetCreated) {
        self.destroyed_targets.remove(&event.target_info.target_id);
    ...
    fn on_detached_from_target(&mut self, event: EventDetachedFromTarget) {
        self.session_event_listeners.remove(&event.session_id);
        // remove the session
    ...
    fn on_target_destroyed(&mut self, event: EventTargetDestroyed) {
        ...
        self.destroyed_targets.insert(event.target_id.clone());
                    ...
                    while let Some(event) = target.advance(now) {
                        match event {
                            ...
                            TargetEvent::AttachedToTarget(event) => {
                                self.event_listeners.start_send((*event).clone());
                                self.on_attached_to_target(event);
                            }
                            TargetEvent::DetachedFromTarget(event) => {
                                self.event_listeners.start_send(event.clone());
                                self.on_detached_from_target(event);
                            }
                        }
                    }
                    ...
                    match msg {
                        Some(msg) => {
                            match msg {
                                ...
                                HandlerMessage::GetAttachedSession(target_id, tx) => {
                                    let state = if self.destroyed_targets.contains(&target_id) {
                                        AttachedSessionState::TargetDestroyed
                                    } else if let Some(target) = self.targets.get_mut(&target_id) {
                                        match target.get_or_create_page() {
                                            Some(page) => AttachedSessionState::Attached(Page::from(page.clone())),
                                            None => AttachedSessionState::NotAttached,
                                        }
                                    } else {
                                        AttachedSessionState::UnknownTarget
                                    };
                                    let _ = tx.send(state);
                                }
                                ...
                                HandlerMessage::AddSessionEventListener(session_id, req) => {
        ...
        loop {
            ...
            while let Poll::Ready(Some(msg)) = pin.from_browser.poll_recv(cx) {
                match msg {
                    ...
                    HandlerMessage::GetAttachedSession(target_id, tx) => {
                        let state = if pin.destroyed_targets.contains(&target_id) {
                            AttachedSessionState::TargetDestroyed
                        } else if let Some(target) = pin.targets.get_mut(&target_id) {
                            match target.get_or_create_page() {
                                Some(page) => {
                                    AttachedSessionState::Attached(Page::from(page.clone()))
                                }
                                None => AttachedSessionState::NotAttached,
                            }
                        } else {
                ...
                if let Some((id, mut target)) = pin.targets.remove_entry(&target_id) {
                    ...
                    while let Some(event) = target.poll(cx, now) {
                        match event {
                            ...
                            TargetEvent::AttachedToTarget(event) => {
                                pin.event_listeners.start_send((*event).clone());
                                pin.on_attached_to_target(event);
                            }
                            TargetEvent::DetachedFromTarget(event) => {
                                pin.event_listeners.start_send(event.clone());
                                pin.on_detached_from_target(event);
                            }
                        }
                        drained += 1;
            ...
            for listeners in pin.session_event_listeners.values_mut() {
                listeners.poll(cx);
            }
...
pub(crate) enum HandlerMessage {
    ...
    GetAttachedSession(TargetId, OneshotSender<AttachedSessionState>),
    AddEventListener(EventListenerRequest),
    AddSessionEventListener(SessionId, EventListenerRequest),
    CloseBrowser(OneshotSender<Result<CloseReturns>>),
}

#[derive(Debug)]
pub(crate) enum AttachedSessionState {
    Attached(Page),
    NotAttached,
    TargetDestroyed,
    UnknownTarget,
